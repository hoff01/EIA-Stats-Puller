...
**One-button Windows use:** install Python 3.11+ and extract the entire download. Double-click `RUN_EIA_STATS.bat`. It automatically creates the environment, installs dependencies when needed, fetches the stats, copies the image to the clipboard, opens the preview and archives the result. On non-release days it fetches the latest published week immediately; on release days it waits for the scheduled release. No separate setup step is required. Keep the Windows session unlocked for clipboard copying. No administrator rights are required. `SETUP_WINDOWS.bat` is an optional setup-only button that does not fetch stats.

**Latest immediately, any day:** `RUN_EIA_STATS.bat -Latest` bypasses the calendar and regenerates the latest live week even if it was already copied before. `-IgnoreSchedule` is an alias for this behavior. This mode may return the previous published week when started before the new release. It polls unavailable/blank responses every 0.5 seconds for up to two minutes; the archive and previous run history do not choose the week. A missing/unavailable calendar also falls back to live latest mode. `-ShowDecision` never fetches stats or changes the clipboard.
...
That runner refreshes the EIA schedule weekly, interprets all listed times in New York/Eastern time, fetches latest published data on non-release days, and on release days waits until the official release time before starting the poll. The standard schedule on the EIA page is `10:30 a.m. Eastern`. Release-day runs use the calendar's explicit reporting week, not the next Friday inferred from prior output. Scheduled duplicate weeks are skipped unless `-Force` is supplied; latest-mode runs always refresh and republish the image. An unavailable-data timeout returns a nonzero exit code instead of being reported as success.
